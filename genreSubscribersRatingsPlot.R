...
sub_rat_data <- working_data |>
  group_by(genre) |>
  summarize(avg_rating = mean(rating),
            mil_subscribers = sum(subscribers)/1000000) |>
  arrange(desc(mil_subscribers)) |>
  mutate(metrics = if_else())

gen_sub_rat_plot <- sub_rat_data |>
  ggplot(aes(x = genre,
             y = metrics,
             fill = if_else(metrics = "sub", webt_green, "brown2"))) +
...
write_rds(gen_sub_rat_plot, "genre_sub_rat.rds")
